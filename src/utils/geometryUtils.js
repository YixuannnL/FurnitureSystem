...
export function collectAtomicGroups(root) {
  ...
  function dfs(node) {
    ...
    const hasGroupChild = node.children.some((c) => !c.isLeaf);
    const hasLeafChild = node.children.some((c) => c.isLeaf);
    /* 只有“至少含 1 个 leaf-子节点”且“没有 group-子节点” 才算原子组 */
    if (hasLeafChild && !hasGroupChild) {
      if (node.path.length) result.push(node.path);
    } else {
