...
function buildStandardConns(role2node) {
  ...
  // }));
  return defs.map(([ra, rb, aa, bb]) => {
    const nodeA = role2node[ra];
    const nodeB = role2node[rb];
    return {
      [nodeA.name]: anchorStr(nodeA.name, aa),
      [nodeB.name]: anchorStr(nodeB.name, bb),
      /* ★ 唯一定位这两个板件 */
      pathA: nodeA.path.join("/"),
      pathB: nodeB.path.join("/"),
    };
  });
}
...
function getThickness(role, role2node) {
  return (
    role2node.left?.dims.width ??
    role2node.right?.dims.width ??
    role2node.front?.dims.depth ??
    role2node.back?.dims.depth ??
    role2node.bottom?.dims.height ??
    3
  );
}
...
function dimsForRole(role, { W, H, D }, t) {
  ...
  if (role === "left" || role === "right")
    return { width: t, height: H, depth: D };
...
function processDrawer(groupNode, meshMap, removeMeshCb, addMeshCb) {
  ...
  if (missing.length) {
    ...
    if (W && H && D) {
      ...
      console.warn(
        `[drawerUtils] <${groupNode.name}> 缺少板件过多，自动装配已跳过`
      );
      return;
    }
  }
  ...
  //   });
  const drawerPrefix = groupNode.path.join("/");
  const kept = store.connections.filter((c) => {
    /* 新格式：有 pathA / pathB —— 用前缀判断 */
    if (c.pathA && c.pathB) {
      return !(
        c.pathA.startsWith(drawerPrefix) && c.pathB.startsWith(drawerPrefix)
      );
    }
    /* 旧格式才退回到“名字集合”判断 */
    ...
    return !(
      ks.length >= 2 &&
      partNamesInDrawer.has(ks[0]) &&
      partNamesInDrawer.has(ks[1])
    );
  });
